...
class TrajComp():
    ...
    def read(self, p, episode, rem, flag):
        ...
        if flag:
            t = self.op([self.ori_traj_set[episode][s], self.ori_traj_set[episode][rem], self.ori_traj_set[episode][m], self.ori_traj_set[episode][e]])
            self.F_ward[m][0] = t
            self.B_ward[m][0] = t
        else:
            t = self.op([self.ori_traj_set[episode][s], self.ori_traj_set[episode][m], self.ori_traj_set[episode][e]])
            self.F_ward[m][0] = t
            self.B_ward[m][0] = t
    ...
    def reward_update(self, episode, rem, label=''):
        ...
        if (rem not in self.start) and (rem not in self.end):
            ...
            b = rem
            c = self.start[rem]
            BEFORE = self.err_seg[(b,c)] #self.op(self.ori_traj_set[episode][b: c + 1])
        ...
        # interval expand right
            ...
            b = rem
            c = self.F_ward[rem][1]
            BEFORE = self.err_seg[(a,b)] #self.op(self.ori_traj_set[episode][a: b + 1])
            NOW = self.op(self.ori_traj_set[episode][a: c + 1])
        ...
        # interval merge
            ...
            c = self.start[b]
            # get values quickly
            BEFORE_1 = self.err_seg[(a,b)] #self.op(self.ori_traj_set[episode][a: b + 1])
            BEFORE_2 = self.err_seg[(b,c)] #self.op(self.ori_traj_set[episode][b: c + 1])
            NOW = self.op(self.ori_traj_set[episode][a: c + 1])
            del self.err_seg[(a,b)]
            del self.err_seg[(b,c)]
            self.err_seg[(a,c)] = NOW            
    ...
    def step(self, episode, action, index, done, label = 'T'):        
        ...
        if LAST_P > self.link_head:
            ...
            #self.op(self.ori_traj_set[episode][self.B_ward[LAST_P][1]: NEXT_P + 1])
            t = self.op([s,m1,m2,e])
        ...
        if NEXT_P < self.link_tail:
            ...
            #self.op(self.ori_traj_set[episode][LAST_P: self.F_ward[NEXT_P][1] + 1])
            t = self.op([s,m1,m2,e])
        ...
        if  label == 'T':
            ...
            _,  self.current = self.err_comp(self.ori_traj_set[episode], self.copy_traj)
            '''
